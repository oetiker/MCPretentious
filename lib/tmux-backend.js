...
export class TmuxBackend extends TerminalBackend {
  ...
  async sendMouseClick(sessionId, x, y, button = 'left') {
    ...
    return await this.client.sendMouseClick(actualSessionId, x, y, button);
  }
  ...
  async sendMouseDrag(sessionId, startX, startY, endX, endY, button = 'left') {
    ...
    return await this.client.sendMouseDrag(actualSessionId, startX, startY, endX, endY, button);
  }

  /**
   * Send mouse scroll to terminal
   * @param {string} sessionId - Target session
   * @param {number} x - X coordinate
   * @param {number} y - Y coordinate
   * @param {string} direction - Scroll direction ('up' or 'down')
   * @param {number} amount - Number of scroll steps
   * @returns {Promise<boolean>} Success status
   */
  async sendMouseScroll(sessionId, x, y, direction, amount = 1) {
    ...
    return await this.client.sendMouseScroll(actualSessionId, x, y, direction, amount);
  }
  ...
  async sendMouseEvent(sessionId, event, x, y, buttonCode) {
    ...
    // Convert to 1-based coordinates for protocol
    const protocolX = x + 1;
    const protocolY = y + 1;
    ...
    const sequence = `\x1b[<${buttonCode};${protocolX};${protocolY}${suffix}`;
